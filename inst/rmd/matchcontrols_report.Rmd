...
```{r echo=FALSE, warning=FALSE, comment=NA, as.is=TRUE, fig.width=10, fig.height=4}

final.matched <- m$match_data %>% filter(cohort == "Case" | k2k_control == TRUE)

CreateTableOne(vars = m$match_variables, strata = c("cohort"), data = final.matched, factorVars = factor_variables)
